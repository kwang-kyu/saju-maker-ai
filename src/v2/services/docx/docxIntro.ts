...
export function lifeStageFlow(name: string) {
  return [
    sectionTitle("인생 흐름 한눈에 보기"),

    guideParagraph(
      `${name}님의 인생 흐름은 한 시기만 따로 보는 것보다 초년, 중년, 장년, 말년의 흐름을 함께 보아야 더 입체적으로 이해할 수 있습니다.`
    ),
    guideParagraph(
      "각 시기마다 중요한 역할과 조심해야 할 선택이 다르기 때문에, 지금 내가 어느 흐름 위에 있는지를 확인하는 것이 중요합니다."
    ),

    divider(),

    subTitle("초년 흐름 · 배움과 기준을 만드는 시기"),
    bodyParagraph(
      "초년에는 환경의 영향을 많이 받으며, 스스로의 기준을 세우기까지 시간이 필요한 흐름입니다."
    ),
    bodyParagraph(
      "이 시기에는 빠른 성공보다 사람을 보는 눈, 돈을 다루는 감각, 일을 지속하는 힘을 배우는 것이 중요합니다."
    ),
    bodyParagraph(
      "핵심 조언: 조급하게 결과를 만들기보다 경험을 쌓고, 나에게 맞는 사람과 일을 구분하는 힘을 길러야 합니다."
    ),

    subTitle("중년 흐름 · 기반을 만들고 성과를 쌓는 시기"),
    bodyParagraph(
      "중년에는 그동안 쌓아온 경험이 현실적인 성과로 이어지는 시기입니다."
    ),
    bodyParagraph(
      "직업, 재물, 가족, 건강의 책임이 함께 커지므로 무리한 확장보다 안정적인 기반을 만드는 것이 중요합니다."
    ),
    bodyParagraph(
      "핵심 조언: 수입을 늘리는 것만큼 지출과 건강을 관리하고, 오래 지속될 수 있는 일의 구조를 만들어야 합니다."
    ),

    subTitle("장년 흐름 · 경험과 신뢰가 자산이 되는 시기"),
    bodyParagraph(
    ...
    bodyParagraph(
      "핵심 조언: 무리하게 새로운 판을 벌이기보다 잘해온 일을 정리하고, 사람들에게 신뢰받는 역할을 만드는 것이 좋습니다."
    ),

    subTitle("말년 흐름 · 지키고 편안하게 정리하는 시기"),
    bodyParagraph(
    ...
    bodyParagraph(
      "핵심 조언: 건강, 생활비, 가족 관계, 거주 안정성을 중심으로 무리한 책임을 줄이고 편안한 리듬을 만드는 것이 좋습니다."
    ),

    divider(),

    subTitle("인생 단계별 핵심 정리"),
    bodyParagraph(
