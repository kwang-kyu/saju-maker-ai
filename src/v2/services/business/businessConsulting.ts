...
${name}님의 사업운은 무조건 크게 벌리는 흐름보다, 작게 검증하고 반복 가능한 구조를 만든 뒤 확장하는 방식이 더 안정적입니다.

이번 상담의 핵심 방향은 ${decision.yearlyKeyword}입니다.
현재 우선순위는 ${decision.priorityArea}이고, 의사결정 성향은 ${decision.decisionStyle}에 가깝습니다.

${decision.finalDirection}

2. 사업을 볼 때 가장 중요한 기준

${name}님에게 중요한 것은 "이 사업이 좋아 보이는가"가 아니라 "내가 오래 감당할 수 있는 방식인가"입니다.

사업은 아이디어보다 구조가 중요합니다.
매출이 생기는 구조, 고객이 다시 찾는 구조, 고정비가 과하지 않은 구조가 먼저 잡혀야 합니다.
...
${name}님은 모든 일을 직접 떠안는 방식보다, 본인이 잘하는 역할을 중심에 두고 나머지는 도구, 외주, 협업, 체크리스트로 분리하는 것이 좋습니다.

사업에서 오래 가려면 잘하는 일과 반복되는 일을 구분해야 합니다.
...
사업은 매출이 커 보여도 광고비, 인건비, 장비비, 임대료, 시간이 빠지면 남는 돈이 없을 수 있습니다.

그래서 매달 반드시 매출, 순이익, 고정비, 재구매율, 소개 고객 수를 확인해야 합니다.

5. 조심해야 할 점

가장 조심할 것은 성급한 확장입니다.

될 것 같다는 느낌만으로 장비, 직원, 광고비, 사무실, 매장을 먼저 늘리면 부담이 커질 수 있습니다.
...
목표는 큰돈을 버는 것이 아니라 팔리는 상품과 다시 찾는 고객을 확인하는 것입니다.
...
순이익이 남고, 고객이 다시 오고, 운영이 버틸 수 있을 때 확장하는 것이 좋습니다.
...
다만 크게 시작하는 것보다 작게 시작해서 숫자로 확인하고, 고객 신뢰를 쌓으면서 키우는 방식이 맞습니다.

${identity.successPoint}

사업의 핵심은 아이디어가 아니라 구조입니다.
돈이 남는 구조, 고객이 다시 오는 구조, ${name}님이 지치지 않는 구조를 만들면 사업운은 훨씬 안정적으로 살아날 수 있습니다.
`.trim();
}
