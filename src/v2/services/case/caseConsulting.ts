...
function detectConsultingPattern(question: string): ConsultingPattern {
  const q = question.replace(/\s/g, "");

  if (/(딸|아들|자녀|부모|엄마|아빠)/.test(q)) {
    return "parentChild";
  }

  if (/(궁합|결혼|연애|남친|여친|배우자|부부)/.test(q)) {
    return "couple";
  }

  if (/(동업|파트너|공동사업)/.test(q)) {
    return "businessPartner";
  }

  if (/(창업|개업|장사|가게|사업시작)/.test(q)) {
    return "startup";
  }

  if (/(돈|재물|금전|수입|투자)/.test(q)) {
    return "money";
  }

  if (/(부동산|토지|아파트|상가|건물|집)/.test(q)) {
    return "realEstate";
  }

  if (/(직장|취업|이직|직업)/.test(q)) {
    return "job";
  }

  if (/(건강|질병|몸|수술)/.test(q)) {
    return "health";
  }

  if (/(사람|인간관계|믿어|친구)/.test(q)) {
    return "relationship";
  }

  if (/(언제|시기|몇살|언제쯤)/.test(q)) {
    return "timing";
  }
...
2. 역할, 지분, 책임 범위를 계약서에 적으세요.
3. 투자금과 운영비를 누가 얼마나 부담하는지 먼저 정하세요.
4. 3개월 테스트 후 계속할지 다시 판단하세요.
5. 서로 불편한 돈 이야기를 피한다면 동업은 미루는 것이 좋습니다.`;
  }

  if (key === "contract") {
    title = "계약 상담";
    reality = "계약은 진행할 수 있으나 좋은 말보다 문서 조건을 먼저 봐야 합니다. 위약, 책임 범위, 해지 조건을 확인해야 합니다.";
    caution = "상대의 말보다 계약서 문장이 우선입니다. 애매한 표현은 나중에 부담이 될 수 있습니다.";
  }
...
[최종 결론]
제 결론은 이렇습니다. ${name}님에게 사업은 가능성이 있습니다. 다만 지금은 크게 벌리는 방식보다 사람, 돈, 역할, 책임 구조를 먼저 세우는 것이 더 중요합니다. 특히 동업이라면 좋은 사람인지보다 돈과 책임 앞에서 믿을 수 있는지를 먼저 확인해야 합니다.`;
}
...
function buildPatternAdvice(
  data: BasicSajuResult,
  pattern: ConsultingPattern,
  question: string
): string | null {
  const contextLine = question
  ? `질문을 보면, 지금 핵심은 "${question}" 자체보다 이 선택이 ${data.name || "의뢰인"}님의 사주 흐름에서 감당 가능한지 보는 것입니다.`
  : "";
...
void contextLine;
      ...
      switch (pattern) {
        case "money":
          return buildMoneyAdvice(data, toQuestionKey("moneyTiming"));
      
        case "startup":
          return buildBusinessAdvice(data, toQuestionKey("businessStart"));
      
        case "businessPartner":
          return buildBusinessAdvice(data, toQuestionKey("partnership"));
      
        case "couple":
          return buildRelationshipAdvice(data, toQuestionKey("marriageTiming"));
      


        case "parentChild":
          return buildRelationshipAdvice(data, toQuestionKey("children"));
      
        case "realEstate":
          return buildRealEstateAdvice(data, toQuestionKey("realEstateBuy"));
      
        case "job":
          return buildJobAdvice(data, toQuestionKey("careerDirection"));
      
        case "health":
          return buildHealthAdvice(data);
      
        case "relationship":
          return buildRelationshipAdvice(data, toQuestionKey("relationshipCleanUp"));
      
        case "timing":
          return buildImportantChoiceAdvice(data, question);
...
const pattern = detectConsultingPattern(questionText);
const patternAdvice = buildPatternAdvice(data, pattern, questionText);
