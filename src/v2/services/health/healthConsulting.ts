...
export function getHealthConsulting(data: BasicSajuResult, birthDate?: string, masterDecision?: ReturnType<typeof buildMasterDecision>): string {
  ...
  ${name}님의 건강 관리는 겁을 내야 하는 흐름이 아니라
  생활 리듬을 다시 정돈해야 하는 흐름입니다.
  
  중요한 것은 병명을 단정하는 것이 아니라
  몸이 언제 피로를 쌓고,
  어떤 습관에서 컨디션이 무너지는지를 알아차리는 것입니다.
  ...
  가능하면 일정한 시간에 먹고,
  너무 단 음식이나 기름진 음식을 줄이고,
  속이 불편했던 음식을 기록해두는 것이 좋습니다.
  ...
  ${name}님의 건강 관리는 겁을 주는 방식이 아니라
  생활을 다시 정돈하는 방식으로 가야 합니다.
