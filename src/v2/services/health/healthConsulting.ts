...
function getHealthAgeStrategy(age: number) {
  if (!age) {
    return "현재 건강운은 나이보다 수면, 식사, 스트레스, 회복 리듬이 얼마나 일정하게 유지되는지를 중심으로 보는 것이 좋습니다. 건강은 특별한 관리보다 매일 반복되는 생활 습관에서 먼저 갈립니다.";
  }

  if (age <= 34) {
    return "20~34세 건강운은 체력이 있다고 무리하기 쉬운 시기입니다. 밤샘, 불규칙한 식사, 과한 카페인, 운동 부족, 스트레스성 피로가 쌓이면 젊은 나이에도 컨디션이 빠르게 무너질 수 있습니다. 이 시기에는 큰 병을 걱정하기보다 수면 리듬, 자세, 식습관, 기본 체력을 만드는 것이 중요합니다.";
  }

  if (age <= 49) {
    return "35~49세 건강운은 일, 가정, 책임이 늘어나면서 회복력이 떨어지기 쉬운 시기입니다. 스트레스성 소화 문제, 체중 변화, 혈압, 혈당, 간 기능, 자세 불균형을 관리해야 합니다. 이 시기에는 바쁘다는 이유로 몸의 신호를 미루지 말고 건강검진과 생활 리듬을 숫자로 확인하는 습관이 필요합니다.";
  }

  if (age <= 64) {
    return "50~64세 건강운은 체력보다 회복력과 예방 관리가 핵심입니다. 근력, 관절, 혈관 건강, 수면의 질, 만성 피로를 함께 봐야 합니다. 무리해서 버티는 습관을 줄이고 정기검진, 걷기, 근력 운동, 식사 조절을 꾸준히 이어갈 때 노후 건강운이 안정됩니다.";
  }

  return "65세 이후 건강운은 큰 운동보다 꾸준한 관리와 낙상 예방, 근력 유지, 식사 조절, 정기검진이 중요합니다. 이 시기에는 무리해서 몸을 단련하기보다 매일 걷고, 잘 먹고, 잘 자고, 몸의 작은 이상을 빨리 확인하는 것이 건강운의 중심입니다.";
}
