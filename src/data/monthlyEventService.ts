export function getMonthlyEventText(params: {
  ...
  const monthPhase =
    month === "1월" || month === "2월"
      ? "기초를 준비하고 방향을 설정하는 시기"
      : month === "3월" || month === "4월"
      ? "활동과 확장이 본격적으로 열리는 시기"
      : month === "5월" || month === "6월"
      ? "책임과 성과가 동시에 커지는 실행의 시기"
      : month === "7월" || month === "8월"
      ? "흐름을 점검하고 조정하는 중간 관리의 시기"
      : month === "9월" || month === "10월"
      ? "관계와 금전 흐름이 현실적으로 드러나는 시기"
      : month === "11월" || month === "12월"
      ? "한 해를 정리하고 다음 흐름을 준비하는 마무리 시기"
      : "월별 흐름을 점검하고 생활 리듬을 조정하는 시기";
  ...
  const elementText =
    monthElement === strongestElementName
      ? `이번 달의 ${monthElement} 기운은 사주에서 이미 강한 ${strongestElementName} 기운과 겹치므로 추진력은 커지지만 과하면 고집과 충돌로 나타날 수 있습니다.`
      : monthElement === weakestElementName
      ? `이번 달의 ${monthElement} 기운은 사주에서 부족한 ${weakestElementName} 기운을 보완해 주는 흐름이므로 약점을 정리하고 균형을 잡기 좋습니다.`
...
정리하면 ${month}은 단순히 좋은 달과 나쁜 달로 보기보다, 들어오는 ${monthTenGod} 기운을 어떻게 활용하느냐에 따라 재물직업관계건강의 결과가 달라지는 달입니다.`;
}
